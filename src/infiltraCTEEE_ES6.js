...
function findRoot(fun, low, hi, funabserr, maxiter) {
  ...
  for (let k = 0; k <= maxiter; k++) {
    if (Math.abs(funxhi - funxlow) < funabserr / 2 && xhi - xlow < xabserr)
      return mid;
...
function qv(C, dP, n) {
  return C * Math.sign(dP) * Math.pow(Math.abs(dP), n);
}
...
function qvComp(Cleak_fach, Cleak_cub, Cven, v, p) {
  ...
  const dP_fach2 = dP_comp(-0.5, v, p); // cp = -0.50 sotavento
  const dP_cub = dP_comp(-0.6, v, p); // cp = -0.60 cubierta
...
function infiltraciones(
  ...
  const Cleak_fach =
    Ao * Co100 * F100to1 + Ah * Ch100 * F100to1 + Apu * Cpu100 * F100to1;
  ...
  const p_R = findRoot(
    (p) =>
      qvComp(Cleak_fach, Cleak_cub, Cven, 4, p).reduce((a, b) => a + b, 0) -
      qv_exh_req,
    -100,
    100,
    1e-8,
  ...
  if (p_R === null) {
    return "Error al calcular las infiltraciones. No se ha podido calcular la presión del recinto";
  }
